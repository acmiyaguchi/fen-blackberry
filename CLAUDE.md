...
- Link is partial-static: `-Wl,-Bstatic -llua -lcurl -lssl -lcrypto -lz -lm
  -Wl,-Bdynamic -lsocket`. Our code, Lua, sysroot `libm.a`, AND the HTTPS/TLS
  stack are baked in — bbnix's static `libcurl.a` over its from-source OpenSSL
  3.x + zlib (flake input `bbnix`, attrs `bb.curl`/`bb.openssl`/`bb.zlib`, store
  paths exported into `.#cross` as `BBNIX_CURL`/`BBNIX_OPENSSL`/`BBNIX_ZLIB`).
  This escapes the device's EOL `libcurl.so.2` / OpenSSL 1.0.x and its stale CA
  store, and avoids a runtime `libgcc_s.so.1` dependency from dynamic `libm.so.2`.
  Only QNX platform libs stay dynamic: `libsocket.so.3` (QNX sockets/getaddrinfo,
  needed now that curl is static) and `libc.so.3` — resolved on-device.
